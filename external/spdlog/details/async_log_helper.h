...
public:
  ...
  async_log_helper(formatter_ptr formatter, const std::vector<sink_ptr> &sinks,
...
private:
  ...
  // the pop time return false if termination of the queue is required
...
inline spdlog::details::async_log_helper::async_log_helper(
    formatter_ptr formatter, const std::vector<sink_ptr> &sinks,
    size_t queue_size, log_err_handler err_handler,
    const async_overflow_policy overflow_policy,
    const std::function<void()> &worker_warmup_cb,
    const std::chrono::milliseconds &flush_interval_ms,
    const std::function<void()> &worker_teardown_cb)
    : _formatter(formatter), _sinks(sinks), _q(queue_size),
      _err_handler(err_handler), _flush_requested(false),
...
inline void spdlog::details::async_log_helper::flush(bool wait_for_q) {
  ...
  if (wait_for_q)
    wait_empty_q(); // return only make after the above flush message was
                    // processed
}
